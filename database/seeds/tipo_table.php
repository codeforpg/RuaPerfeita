                ...
                if(strpos($linha,'Point') === 0){
                    ...
                    switch ($linha_array[6]){
                        case 'Controlador de Velocidade':
                            ...
                            break;
                        case 'Rotatoria':
                            $tipo = \App\Tipo::where('nome','Rotatorio')->first()->id_tipo;
                            break;
                        case 'Faixa Elevada':
                            $tipo = \App\Tipo::where('nome','Faixa Elevada')->first()->id_tipo;
                            break;
                        case 'Semaforo':
                            $tipo = \App\Tipo::where('nome','Semaforo')->first()->id_tipo;
                            break;
                        case 'Lombada Eletronica':
                            $tipo = \App\Tipo::where('nome','Lombada')->first()->id_tipo;
                            break;
                    }
                    $pin = [];
                    $pin['lat'] = substr_replace($linha_array[7], '.', 3, 0);
                    $pin['long'] = substr_replace($linha_array[8], '.', 3, 0);
                    $pin['tipo'] = $tipo;
                    \App\Pin::create($pin);
                }
            }
